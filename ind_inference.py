...
def main(cfg: DictConfig):
    ...
    check_dir(path_logs)
    name_file = os.path.join(path_logs, f"ind_results.npy")
    ...
    all_ind, best_model, first_best_data, first_loop = chain.loop(cfg.dataset, DATA_PB_DIR, L = cfg.L, N_max=0, verbose = True)
    ALL_ind[:first_loop+1,:] = all_ind
    ...
    for ind in range(n_ex):
        ...
        ALL_ind[first_loop+1:first_loop+1+best_nloop,ind,:,:] = all_ind[:,0,:,:]
